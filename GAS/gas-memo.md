...
- [Google Apps Scriptメモ](#google-apps-scriptメモ)
    - [目次](#目次)
    - [概要](#概要)
    - [できること](#できること)
    - [手順（スプレッドシート）](#手順スプレッドシート)
    - [基礎文法（スプレッドシート操作）](#基礎文法スプレッドシート操作)
        - [・最初の表示とカスタム関数](#・最初の表示とカスタム関数)
        - [・カスタム関数（複数のセル使用）](#・カスタム関数複数のセル使用)
        - [・シートの初期化・値の追加・その他関数](#・シートの初期化・値の追加・その他関数)
        - [・ループ処理による値追加](#・ループ処理による値追加)
        - [・処理時間の取得・処理時間の高速化](#・処理時間の取得・処理時間の高速化)
        - [・判定処理](#・判定処理)
        - [・ボタン実装](#・ボタン実装)
        - [※判定処理（上記）の中身を、スプレッドシート内で操作。](#※判定処理上記の中身をスプレッドシート内で操作)
        - [・確認メッセージの実装](#・確認メッセージの実装)
        - [※スクリプトが押されたときに、確認メッセージを出して、押し間違いを防ぐ。](#※スクリプトが押されたときに確認メッセージを出して押し間違いを防ぐ)
        - [・メニューの追加](#・メニューの追加)
        - [※タブメニューに、関数メニューを追加。](#※タブメニューに関数メニューを追加)
    - [タスク管理作成（シート・フォーム操作）](#タスク管理作成シート・フォーム操作)
        - [・手順](#・手順)
